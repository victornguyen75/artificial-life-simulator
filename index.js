...
const setupGUI = () => {
  ...
  configFolder
    .add(settings, 'time_scale', 0.1, 5, 0.01)
    .name('Time Scale')
    .listen();
  configFolder
  ...
  configFolder
    .add(settings, 'wallRepel', 0, 100, 1)
    .name('Wall Repel')
    .listen();
  ...
  for (const atomColor of settings.colors) {
    const colorFolder = settings.gui.addFolder(
      `Rules: <font color=\'${atomColor}\'>${atomColor.capitalise()}</font>`,
    );
    for (const ruleColor of settings.colors) {
      colorFolder
        .add(settings.rules[atomColor], ruleColor, -1, 1, 0.001)
        .name(
          `<-> <font color=\'${ruleColor}\'>${ruleColor.capitalise()}</font>`,
        )
        .listen()
        .onFinishChange((v) => {
          flattenRules();
    ...
    colorFolder
      .add(settings.radii, atomColor, 1, MAX_RADIUS, 5)
      .name('Radius')
      .listen()
      .onFinishChange((v) => {
        flattenRules();
...
function loadSeedFromUrl() {
  ...
  if (hash != undefined && hash[0] == '#') {
    let param = Number(hash.substring(1)); // remove the leading '#'
    if (isFinite(param)) {
      settings.seed = param;
...
function setNumberOfColors() {
  ...
  for (let i = 0; i < settings.numColors; ++i) {
    settings.colors.push(PREDEFINED_COLORS[i]);
  }
}
...
let clusters = [];
function newCluster() {
  return [randomX(), randomY(), MAX_RADIUS, 'white'];
}
...
function finalizeClusters(accums) {
  for (let i = 0; i < clusters.length; ++i) {
    ...
    if (a[0] > MIN_CLUSTER_SIZE) {
      const norm = 1 / a[0];
      const new_r = 1.1 * Math.sqrt(a[3] * norm); // with 10% extra room
...
function trackClusters() {
  ...
  for (let pass = maxKMeanPasses; pass >= 0; --pass) {
    ...
    for (const c of atoms) {
      ...
      if (best >= 0 && best_d2 < MAX_RADIUS * MAX_RADIUS) {
        accums[best][0] += 1;
        accums[best][1] += c[0];
        accums[best][2] += c[1];
...
function exploreParameters() {
  if (exploration_timer <= 0) {
    ...
    if (mulberry32() >= 0.2) {
      ...
      settings.radii[c1] = 1 + Math.floor(mulberry32() * MAX_RADIUS);
    }
    flattenRules();
...
let total_v; // global velocity as a estimate of on-screen activity
